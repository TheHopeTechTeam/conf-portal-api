...
from portal.schemas.thehope_ticket import (
    ...
    TheHopeTicketType, TheHopeTicketTypesResponse,
)
...
class TheHopeTicketProvider:
    ...
    @distributed_trace()
    async def get_tickets_by_email(self, user_email: str) -> Optional[TheHopeTicketsListResponse]:
        """
        Fetch tickets by user email, parse raw response into TheHopeTicketsListResponse.
        :param user_email:
        :return:
        """
        raw = await self._service.get_ticket_by_email(user_email)
        if raw is None:
            return None
        return TheHopeTicketsListResponse.model_validate(raw)

    @distributed_trace()
    async def get_ticket_list_by_email(self, user_email: str) -> Optional[TheHopeTicket]:
        """
        Get tickets by email and return only the list of ticket objects (processed for convenience).
        :param user_email:
        :return:
        """
        response = await self.get_tickets_by_email(user_email)
        if response is None:
            return None
        return response.docs[0]

    @distributed_trace()
    async def check_in_ticket(self, ticket_id: UUID) -> None:
        """
        Check in a ticket by id; returns objectified response or None on failure.
        :param ticket_id:
        :return:
        """
        try:
            await self._service.check_in_ticket(ticket_id)

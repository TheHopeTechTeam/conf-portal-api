...
**GitHub Actions secrets for Render ARQ worker (same Docker image as API):** add **`RENDER_WORKER_SERVICE_ID_STG`** (Render service id for the staging worker, e.g. `srv-...`). For semver releases, **`RENDER_WORKER_SERVICE_ID`** is optional: when set, `release.yml` also PATCHes the production worker image; when unset, only the STG worker image is updated.
